	...
	for (k=1;k<=M*N;k++)
	{
		...
		if (j==1)
		{
			...
			if (l>1.8)
			{
				...
				for (i1=p;i1>=1;i1--)
				{
					xx1[i][i1]=xx1[i][i1-1]; /*将后一个链段向前移动*/
					yy1[i][i1]=yy1[i][i1-1];
					zz1[i][i1]=zz1[i][i1-1];
					jj11[tfX(xx1[i][i1])][tfY(yy1[i][i1])][tfZ(zz1[i][i1])]++;
				} /*由于第i个链段取代了第（i-1）个链段的位置，而实际上它是第i个链段，所以jj要+1*/
				ii11[cx][cy][cz]=i;
				jj11[cx][cy][cz]=1;
				//运动后的坐标
			...
			if (hindbend(a,b,c,x,y,z,(int)(l*l+0.2))==1) /*hindbend函数返回值是1的话，说明会打断已有的键，是不允许的运动*/
			continue;
		...
		{ /*左边的点要跟着动*/
			...
			{ /*左边的链段向右动了起来*/
				...
				yy[i][i1]=yy[i][i1+1];
				zz[i][i1]=zz[i][i1+1];
				jj[tfX(xx[i][i1])][tfY(yy[i][i1])][tfZ(zz[i][i1])]--; /*由于i1的坐标已经被赋值成了（i+1）的了，而实际的链段标号却没变，故jj函数的值（原来（i+1）链段的标号）要减一*/
			}
			xx[i][j]=xc;
			yy[i][j]=yc;
			zz[i][j]=zc;
	...
	if (a < b)
	{
		for (i1 = a; i1 <= b - 2; i1++)
		{
			...
			if (r == 2)
			{
				if (x == 0 && x1 != x2)
				{
					if (ii[x1][y3][z1] == ii[x1][y1][z3] && /*防止形成的新键打断原来存在的旧键*/
					abs(jj[x1][y3][z1] - jj[x1][y1][z3]) == 1)
						continue;
					if (y2 == y1 && z2 == z3 && ii[x1][y1][z3] == ii[x2][y3][z1]
							&& abs(jj[x1][y1][z3] - jj[x2][y3][z1]) == 1) /* 面对角线会打断体对角线！！！！(原来的一根旧键由一个面摆动到另一个面，会打断体对角线，这是不允许的运动）*/
						continue;
	...
	if (l == 2)
	{
		if (cx == a && cx != x)
		{
			...
			if (y == cy && z == c && abs(ii[a][cy][c] - ii[x][b][cz]) == 0
					&& abs(jj[a][cy][c] - jj[x][b][cz]) == 1) /*同样的疑惑：面对角线的建立会影响体对角线吗？（答案见上）*/
				return (1);
		}
		if (cy == b && cy != y)
		{
